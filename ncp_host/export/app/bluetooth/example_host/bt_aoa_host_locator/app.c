...
// The following headers are added by Uygar
...
void app_init(int argc, char *argv[])
{
  ...
  // Initialize UART between Pi and STM32
  open_serial_port();
  ...
  sc = ncp_host_init(); // UYGAR_LOG: initialized UART and TCP and such (maybe we can do this on STM side without this code needed)
...
void app_deinit(void)
{
  ...
  // UART deinitialize between Pi and STM32
  close_serial_port();
...
void aoa_cte_on_iq_report(aoa_db_entry_t *tag, aoa_iq_report_t *iq_report)
{
  ...
  // Variables for UART to Pi
  int32_t angle_4bytes;
  int16_t angle_2bytes;
  uint8_t uart_tx_buff[2];
  // char *payload;
  // char *topic_template;
  // char *topic;
  // size_t size;
  ...
  if (report_mode == IQ_REPORT) {
    ...
    // printf("ANGLE: %f\n", angle.azimuth); // UYGAR_LOG: ANGLES ARE PRINTED HERE
    ...
    // If the angle is received successfully, send the angle over UART to Pi (UYGAR_LOG)
    if (ec == SL_RTL_ERROR_SUCCESS)
    {
      angle_4bytes = (int32_t)angle.azimuth; // turn float into int
      angle_2bytes = (int16_t)angle_4bytes; // turn int into short int (range is -180 to 180 for the angle)
      memcpy(uart_tx_buff, &angle_2bytes, 2); // copy the angle into the uart transmission buffer
      
      file_write_data(g_fd, uart_tx_buff, 2); // Send over UART to Pi
      printf("Sent %hd\n", ((int16_t *)uart_tx_buff)[0]);
    }
    
    // 1111 1111 1111 1111
    
    // Compile payload (UYGAR_LOG: NO NEED TO SERIALIZE THE AOA INFO, THIS IS FOR SENDING IT TO MQTT AS A JSON AND WE DO NOT NEED IT)
    // sc = aoa_serialize_angle(&angle, &payload);
  }
  // UYGAR_LOG: NO NEED FOR THIS CHECK IN OUR SCENARIO
  //app_assert(sc == SL_STATUS_OK,
   //          "[E: 0x%04x] Failed to serialize the payload." APP_LOG_NL,
     //        (int)sc);

  // Compile topic (UYGAR_LOG: NO NEED FOR THIS AS MQTT WON'T BE USED, CAN TAKE THESE OUT)
  ...
  // Send message (UYGAR_LOG: IN OUR CASE, WE SHOULD BE SENDING THE AZIMUTH TO STM32)
  // sc = mqtt_publish(&mqtt_handle, topic, payload, false); // IF THE FOLLOWING 2 LINES ARE COMMENTED, MQTT WON'T BE PUBLISHED
  //app_assert_status(sc);
